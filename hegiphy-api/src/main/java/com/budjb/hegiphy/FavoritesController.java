...
public class FavoritesController {
    ...
    public ResponseEntity<?> get(@PathVariable("id") String id, @AuthenticationPrincipal Principal principal) {
        Optional<Favorite> result = loadFavorite(id, principal.getName());
        if (result.isEmpty()) {
    ...
    /**
     * Loads a favorite Giphy image instance and ensures that the user who
     * created it matches the given user ID.
     *
     * @param id   ID of the Giphy image to load.
     * @param user ID of the user to correlate.
     * @return An {@link Optional<Favorite>} that will contain the favorite instance
     * iff the favorite exists and the given user correlates with that
     * contained within the record.
     */
    private Optional<Favorite> loadFavorite(String id, String user) {
        Optional<Favorite> result = favoriteRepository.findByGiphyIdAndUser(id, user);
        if (result.isEmpty()) {
            return result;
        }

        Favorite favorite = result.get();
        if (!favorite.getUser().equalsIgnoreCase(user)) {
