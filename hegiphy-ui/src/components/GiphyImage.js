...
import { faHeart as fasHeart, faTag, faTimes } from '@fortawesome/free-solid-svg-icons';
...
const GiphyImage = props => {
  ...
  const {image} = props;
  const conf = image.images.fixed_width;
  const {id} = image;
  ...
  return (
    <div className="giphy-image" style={{ width: `${conf.width  }px` }}>
      <img src={conf.url} width={conf.width} height={conf.height} alt={image.title} />
